...
class OVMS_model_info:
    ...
    def raw_infer(self, inputs, timeout:float=10.0):
        ...
        for bname, bval in inputs.items():
            ...
            if idx == -1:
                self.logger.error('Input blob "{}" is not found.'.format(bname))
                return None
...
class OpenVINO_Model_Server:
    ...
    def open_model(self, model_name:str, timeout:float=10.0):
        ...
        for key in serving_inputs.keys():
            name = serving_inputs[key].name
            shape = [dim.size for dim in serving_inputs[key].tensor_shape.dim]
        ...
        for key in serving_outputs.keys():
            ...
            shape = [dim.size for dim in serving_outputs[key].tensor_shape.dim]
